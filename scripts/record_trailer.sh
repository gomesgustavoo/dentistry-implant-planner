#!/usr/bin/env bash
# Bring up the stack, drive the trailer storyboard, assemble the raw body.
#
...
#
#   scripts/record_trailer.sh     raw body + trailer-beats.json
#   scripts/trailer_overlays.py   the plates, cards and social frames
#   scripts/build_trailer.sh      composites all three into the deliverables
set -euo pipefail
...
FRAMES="$WORK/frames"
OUT="${TRAILER_RAW:-$WORK/body-raw.mp4}"
...
TOUR_FPS="${TOUR_FPS:-10}" TOUR_CASE="${TOUR_CASE:-}" \
  node scripts/record_trailer.mjs | tee "$WORK/beats.log"
RC=${PIPESTATUS[0]}
set -e
...
tail -1 "$WORK/beats.log" > "$WORK/trailer-beats.json"
...
./venv/bin/python -c "
import json,sys
d=json.load(open('$WORK/trailer-beats.json'))
...
ls -lh "$OUT"
echo "beats -> $WORK/trailer-beats.json"
